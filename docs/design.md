...
The composer is Psi's own modal state machine over a `ropey` text buffer, structured from day one around Vim's grammar — `[count] operator [count] (motion | text object)` — so later Vim features are table entries rather than rewrites. Existing widget crates are not used: they own rendering, and none models the grammar. modalkit is the semantic reference, and its core is the fallback dependency if hand-rolling stalls. The MVP needs only reliable multiline editing, history, and basic normal/insert modes; once Vim support grows past that, correctness is checked differentially against headless Neovim — identical keystrokes into both, comparing buffer, cursor, mode, and registers including register type. Insert mode also carries the readline bindings a terminal prompt is expected to have — Ctrl-A/E, Ctrl-U/K/W — each defined as a motion the grammar already computes, so a prompt and a Vim motion never disagree about where a word starts.

Everything the TUI offers beyond projecting the event stream is a client convenience, and it stays on the client. Slash commands (`/new`, `/resume`, `/fork`, `/help`, `/quit`) are typed into the composer and turn into the protocol commands that already exist, or into nothing at all. The `@` file picker walks the workspace itself under a subsequence scorer of its own, because completing a path is not session state and there is no command for it. Prompt history is one file beside the session logs, one JSON string per line and named so the session store's listing ignores it. None of the three adds an event, a command, or a durable record.

The status row carries news only — a running turn, a cancellation in flight, which branch the head is on when there is more than one — because a row that is always full is a row nobody reads; the keys it might otherwise list live in `/help` and on the header of the overlay that wants them. A tool call prints the wall time of its result on the line that names the call, which means a call is held back until its result arrives: terminal scrollback only grows, so a duration can reach the line that names the call only by waiting for it. That number is the per-call latency the speculation work exists to move.
...
A task can also be a recording, which is what makes a predictor's hit rate meaningful: a hand-written script judges guesses against argument choices no model made, while a recording is a real model's own trajectory. The recorder (`psi-bench --record`) drives the live agent once over a snapshotted fixture and keeps the fixture, the session log, the final workspace, and the run's trace; the loader rebuilds the model script from the session's items — a streamed item after a tool result begins the next response, since a response's results are the last items it appends — and refuses interrupted recordings. Replay is timed by the recording rather than by injected distributions: each response carries its real generation delay and each turn the token usage its trace record reports (usage is never an item, so the trace is where it lives), and each call is stretched to its recorded duration — the replayed execution counts toward it, since the recorded time already contains the live one. Timing is stateless: an authoritative call replays under the recording's own call id and takes exactly the time that call took live, while a guess — an id the recording never issued — falls back to the call's canonical identity, so a speculative execution costs what the call it stands for cost live and no measurement can be shifted by how many guesses ran or what became of them. Success is final-state equality: the replayed workspace must end byte-for-byte where the live run ended. Only fixture-run recordings replay — a session whose workspace state is not reproducible is not a benchmark — and a recording must be made outside any enclosing build workspace: `exec` is root-exempt by design, so a bare `cargo test` in a fixture nested under a repository ascends to the enclosing manifest and records a trajectory about the wrong project — and one recording is one sample of a stochastic model, mostly single-call rounds while requests disable parallel tool calls; hand-written tasks remain the controlled instruments beside these realistic ones.
